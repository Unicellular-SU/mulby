...
export function useIntools(pluginId?: string) {
  return useMemo(() => ({
    ...
    scheduler: {
      ...
      cancel: (taskId: string) => window.intools?.scheduler?.cancelTask(taskId),
      pause: (taskId: string) => window.intools?.scheduler?.pauseTask(taskId),
      resume: (taskId: string) => window.intools?.scheduler?.resumeTask(taskId),
      list: (filter?: { status?: string; type?: string }) => window.intools?.scheduler?.listTasks(filter),
      get: (taskId: string) => window.intools?.scheduler?.getTask(taskId),
