...
interface SharpExecutePayload {
    input?: string | Buffer | { create?: any; text?: any } | any[]
...
export function registerSharpHandlers() {
    // 执行 Sharp 操作链
    ipcMain.handle('sharp:execute', async (_, payload: SharpExecutePayload) => {
        ...
        try {
            ...
            if (input === undefined) {
                ...
                instance = sharp(Buffer.from(input as any), options)
            } else if (Array.isArray(input)) {
                console.log('[Sharp] 创建: 数组')
                instance = sharp(input as any, options)
            ...
            for (const { method, args } of operations) {
                ...
                // 终结方法返回 Promise
                if (TERMINAL_METHODS.includes(method as any)) {
                    ...
                    console.log('[Sharp] 结果 constructor:', finalResult?.constructor?.name)
                    ...
                    if (method === 'metadata' && typeof finalResult === 'object') {
                        ...
                        for (const [key, value] of Object.entries(finalResult)) {
                            if (Buffer.isBuffer(value)) {
                                ...
                                serializable[key] = (value as Buffer).toString('base64')
                            } else {
                    ...
                    if (method === 'toFile' && typeof finalResult === 'object') {
            ...
            console.error('[Sharp] 错误:', error.message)
            console.error('[Sharp] 堆栈:', error.stack)
            throw new Error(`Sharp 操作失败: ${error.message}`)
        }
    })
