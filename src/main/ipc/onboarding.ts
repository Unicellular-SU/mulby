...
export function registerOnboardingHandlers(
  ...
  // 获取引导所需的当前设置
  ipcMain.handle('onboarding:getSettings', () => {
  ...
  // 更新快捷键
  ipcMain.handle('onboarding:updateShortcut', (_event, action: string, accelerator: string) => {
  ...
  // 更新主题
  ipcMain.handle('onboarding:updateTheme', (_event, mode: string) => {
  ...
  ipcMain.handle('onboarding:updateAiProvider', (_event, provider: AiProviderConfig) => {
