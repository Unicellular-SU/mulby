...
function assertGeolocationPermission(sender: Electron.WebContents): void {
  permissionManager.ensureCallerAccessPluginPermissions(sender, ['geolocation'])
}
...
export function registerGeolocationHandlers() {
  ...
  // 获取位置权限状态
  ipcMain.handle('geolocation:getAccessStatus', (event) => {
  ...
  // 检查是否可以获取位置
  ipcMain.handle('geolocation:canGetPosition', (event) => {
    assertGeolocationPermission(event.sender)
  ...
  // 打开系统位置设置
  ipcMain.handle('geolocation:openSettings', (event) => {
    assertGeolocationPermission(event.sender)
