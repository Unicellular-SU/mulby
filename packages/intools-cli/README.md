...
InTools CLI 内置了 AI Planning 系统，帮助你将复杂任务分解为可执行的子任务，并跟踪执行进度。

### 触发方式

**1. 自动检测（推荐）**

当你输入的任务被检测为中等或复杂时，系统会自动提示：
...
- 系统会调用 AI 分析你的任务描述
- AI 会判断任务复杂度并给出原因
- 粘贴错误信息让 AI 修复会被正确识别为简单任务

示例：
```
📊 复杂任务（预估 6 个步骤）
   原因：需要设计数据库结构、实现 API 接口、编写前端页面，涉及多个模块
💡 建议：此任务可能需要规划。
```
...
- **Plan（计划）**：包含目标和一系列任务的执行方案
- **Task（任务）**：具体的可执行步骤，包含标题、描述、优先级、依赖关系等
- **Template（模板）**：可复用的任务结构，内置 `feature`、`bugfix`、`refactor` 三种模板
...
| 命令 | 说明 |
|------|------|
| `/plan` 或 `/plan show` | 显示当前计划 |
| `/plan edit` | 交互式编辑计划 |
| `/plan clear` | 清除当前计划 |
| `/plan resume` | 恢复已保存的计划 |
| `/plan approve` | 批准草稿计划 |
| `/plan save [name]` | 保存为模板 |
| `/plan load [name]` | 从模板加载 |
| `/plan deps` | 显示依赖树 |
| `/plan validate` | 验证计划依赖 |

#### `/progress` - 进度查看

| 命令 | 说明 |
|------|------|
| `/progress` | 显示进度概览（含进度条）|
| `/progress detail` | 显示详细进度 |
| `/progress export` | 导出 Markdown 报告 |
| `/progress json` | 导出 JSON 报告 |

#### `/task` - 任务控制

| 命令 | 说明 |
|------|------|
| `/task next` | 显示下一个待执行任务 |
| `/task skip [id]` | 跳过指定任务 |
...
| `/template use [name]` | 使用模板创建计划 |
| `/template delete [name]` | 删除自定义模板 |
| `/template builtin` | 查看内置模板详情 |

### 内置模板

| 模板名 | 用途 | 典型步骤 |
|--------|------|----------|
| `feature` | 新功能开发 | 分析需求 → 设计方案 → 实现功能 → 编写测试 → 审核完善 |
| `bugfix` | Bug 修复 | 复现问题 → 定位原因 → 实现修复 → 添加回归测试 |
| `refactor` | 代码重构 | 分析代码 → 设计结构 → 补充测试 → 增量重构 → 验证清理 |
...
# 6. 将常用计划保存为模板
/plan save my-workflow
```

> 📖 **详细文档**: 参见 [`docs/ai-planning-system.md`](../../docs/ai-planning-system.md)
