...
Mulby CLI 现在支持多个 AI 供应商，并允许在会话中灵活切换供应商和模型。
...
首次运行 `mulby create --ai` 时，如果检测到旧配置，会提示你配置新格式。
